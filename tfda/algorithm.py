...
import numpy as np
from models import classifier, Temporal_Imputer, masking, get_distances, soft_k_nearest_neighbors, \
    refine_predictions, eval_and_label_dataset, update_labels, tf_encoder
from loss import EntropyLoss, CrossEntropyLabelSmooth, evidential_uncertainty, evident_dl, contrastive_loss, \
    TEntropyLoss
from scipy.spatial.distance import cdist
from torch.optim.lr_scheduler import StepLR
from copy import deepcopy
from moco import *
from model import TFMPTF
import math
...
class SFTSDA(Algorithm):
    ...
    def pretrain(self, src_dataloader, avg_meter, logger, trg_dataloader):

        for epoch in range(1, self.hparams["num_epochs"] + 1):
            for step, ((src_x, src_y, _, _, _, _, src_xf, _, _, _),(trg_x, _, _, _, _, _, trg_xf, _, _, _)) in enumerate(zip(src_dataloader,trg_dataloader)):

                noise = trg_x.mean(dim=0, keepdim=True) - src_x  # close to trg
                obs_magnitude = src_x.abs().mean(dim=(1, 2), keepdim=True)
                noise_magnitude = noise.abs().mean(dim=(1, 2), keepdim=True)
                ratio = obs_magnitude / (noise_magnitude + 1e-8)  # (Batch, 1, 1)
                src_x += noise * 0.05 * ratio

                noise_f = trg_xf.mean(dim=0, keepdim=True) - src_xf  # close to trg
                obs_magnitude = src_xf.abs().mean(dim=(1, 2), keepdim=True)
                noise_magnitude = noise_f.abs().mean(dim=(1, 2), keepdim=True)
                ratio_f = obs_magnitude / (noise_magnitude + 1e-8)  # (Batch, 1, 1)
                src_xf += noise_f * 0.05 * ratio_f
    ...
    def update(self, trg_dataloader, trg_test_dataloader, src_test_dataloader, avg_meter, logger, num_neighbors,
        ...
        if self.hparams["ad_num_epochs"] is None:
            self.hparams["ad_num_epochs"] = self.hparams["num_epochs"]
        # obtain pseudo labels
        for epoch in range(1, self.hparams["ad_num_epochs"] + 1):
            ...
            for step, (trg_x, y, trg_idx, src_x_strong, src_x_strong2, x, x_f_weak, x_f_strong, x_f_strong2,
                ...
                '''
                loss = mu_r * trg_cls_loss + mu_c * loss_ctr + mu_u * loss_ur + mu_s * loss_sd
            ...
            logger.debug(f'[Epoch : {epoch}/{self.hparams["ad_num_epochs"]}]')
            for key, val in avg_meter.items():
                logger.debug(f'{key}\t: {val.avg:2.4f}')
            logger.debug(f'-------------------------------------')
