...
class LinearSDE(torch.nn.Module):
    ...
    def parallel_compute(self, init, E, Z, obs_times):
        alphas = torch.vmap(lambda u: self.B(u))(
            Z)  # We assume that the control has already pre-computed  [ alphas := E.t() @ alphas ]
        self.alphas = alphas
        batch_size, seq_len, feat_dim = alphas.shape
        alphas_flat = alphas.reshape(-1, feat_dim)  # [15000, 288]
        obs_times_flat = obs_times.reshape(-1, 1)  # [15000, 1]
        mats_A_all, mats_B_all = self.get_matrix(alphas_flat, obs_times_flat)
        mats_A = mats_A_all.reshape(batch_size, seq_len, -1)  # [500, 30, ...]
        mats_B = mats_B_all.reshape(batch_size, seq_len, -1)
        ...
        means = torch.vmap(lambda mean: self.M(torch.vmap(lambda X: E(X))(mean)))(
            xs_mean + init_mean) + history  # Adding history here enhanced the performance

        with torch.no_grad():
            A_mat_dynamic = mats_A[..., :self.ld]
            dynamic_weights = torch.exp(-torch.abs(A_mat_dynamic[..., :self.ld]).mean(dim=-1, keepdim=True))
        means = means * dynamic_weights
    ...
    def forward(self, obs, obs_times, obs_valid, mask_obs, n_samples=3, epoch=None):
        ...
        Z, y_observed = self.encoder(obs, obs_times_, obs_mask=mask_obs,
                                     event_mask=obs_valid)  # Assume q_phi as dirac delta (do not sampling the latent obs.)
        ...
        lyapunov_loss = torch.mean(torch.abs(alphas[:, 1:] - alphas[:, :-1]))
        spectral_loss = torch.mean(torch.abs(torch.fft.rfft(means, dim=1).abs()[:, 1:] -
                                             torch.fft.rfft(y_observed * observed_stamps, dim=1).abs()[:, 1:]))
        L_alpha = self.lamda_1 * (0.7 * lyapunov_loss + 0.3 * spectral_loss) + self.lamda_2 * neg_log_potential
        return Y, L_alpha
