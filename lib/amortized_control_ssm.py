            ...
            for _, data in enumerate(train_dl):
                ...
                if self.dataset == 'pendulum':
                    if self.task == 'regression':
                        ...
                        obs, truth, obs_valid, obs_times, mask_truth = [j.to(self.device).to(torch.float32) for j in
                                                                        data]
                        mask_obs = None
                    ...
                    obs, truth, obs_valid, obs_times, mask_truth, mask_obs = [j.to(self.device).to(torch.float32) for j
                                                                              in data]
    ...
    def eval_func(self, dl):
        ...
        for _, data in enumerate(dl):
            ...
            if self.dataset == 'pendulum':
                ...
                obs, truth, obs_valid, obs_times, mask_truth, mask_obs = [j.to(self.device).to(torch.float32) for j in
                                                                          data]
                if self.task == 'extrapolation':
                    obs, obs_valid = adjust_obs_for_extrapolation(obs, obs_valid, obs_times, self.cut_time)
            ...
            if self.dataset == 'pendulum' and self.task == 'interpolation':
                ...
                eval_mse = MSE_(truth.flatten(start_dim=2), mean.flatten(start_dim=3),
                                mask=mask_truth.flatten(start_dim=2)) * batch_len
            elif self.task == 'classification':
            ...
            if self.task == 'extrapolation' or self.dataset == 'pendulum':
                ...
                if self.dataset == 'pendulum' and self.task == 'interpolation':
                    ...
                    impute_mse = MSE_(truth.flatten(start_dim=2), mean.flatten(start_dim=3),
                                      mask=mask_impute.flatten(start_dim=2)) * batch_len
